const colors = {
  background: "var(--background)",
  foreground: "var(--foreground)",


  primary: "var(--primary)", // Primary
  secondary: "var(--secondary)", // Secondary

  blackC: {
    DEFAULT: "var(--blackC)", // Main black
    light: "var(--blackC-light)", // Light black
    dark: "var(--blackC-dark)", // Dark black
  },
  whiteC: {
    DEFAULT: "var(--whiteC)", // Main white
    light: "var(--whiteC-light)" // Light white
  },
  greenC: {
    DEFAULT: "var(--greenC)", // Main green
    light: "var(--greenC-light)", // Light green
    light2: "var(--greenC-light2)", // Light2 green
    lighter: "var(--greenC-lighter)", // Lighter green
    lightest: "var(--greenC-lightest)", // Lightest green
    lightPale: "var(--greenC-lightPale)", // Light pale green
    lightPale2: "var(--greenC-lightPale2)", // Light pale 2 green
    dark: "var(--greenC-dark)", // Dark green
    darker: "var(--greenC-darker)", // Darker green
    darkPale: "var(--greenC-darkPale)", // Dark pale green
  },
  grayC: {
    DEFAULT: "var(--grayC)", // Main gray
    light: "var(--grayC-light)", // Light gray
    dark: "var(--grayC-dark)", // Dark gray
  },
  pinkC: {
    DEFAULT: "var(--pinkC)", // Main pink
    light: "var(--pinkC-light)", // Light pink
    lightVibrant: "var(--pinkC-lightVibrant)", // Light vibrant pink
  },
  purpleC: {
    DEFAULT: "var(--purpleC)", // Main purple
  },
  blueC: {
    DEFAULT: "var(--blueC)", // Main blue
    dark: "var(--blueC-dark)", // Dark blue
    darkPale: "var(--blueC-darkPale)", // Dark pale blue
    darkPale3: "var(--blueC-darkPale3)", // Dark pale 3 blue
    darkPale4: "var(--blueC-darkPale4)", // Dark pale 4 blue
    darkPale2: "var(--blueC-darkPale2)", // Dark pale 2 blue
    vibrant: "var(--blueC-vibrant)", // Vibrant blue
    light: "var(--blueC-light)", // Light blue
    green: "var(--blueC-green)", // Green blue
  },
  orangeC: {
    DEFAULT: "var(--orangeC)", // Main orange
    dark: "var(--orangeC-dark)", // Dark orange
    light: "var(--orangeC-light)", // Light orange
  },
};


export default colors;