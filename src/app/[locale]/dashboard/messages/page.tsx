      ...
      <div className="flex flex-col gap-y-8 my-10">
        {messages.length !== 0 ? // checks if messages is empty or not to show the proper content
          currentMessages.map((message) => (
            <div key={message.id} onClick={() => setIsChatBoxOpen(true)}>
              <MessageCard selected={message.selected} name={message.name} time={message.time} date={message.date} content={message.content} />
            </div>
          ))
